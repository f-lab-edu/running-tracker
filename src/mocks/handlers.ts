...
export const handlers = [
  // 러닝 목록 조회
  http.get('/api/running', async ({ request }) => {
    await delay(500)
  ...
  // 러닝 상세 조회
  http.get('/api/running/:id', async ({ params }) => {
    await delay(300)
    const { id } = params
  ...
  // 러닝 생성
  http.post('/api/running', async ({ request }) => {
    await delay(500)
  ...
  // 러닝 수정
  http.put('/api/running/:id', async ({ request, params }) => {
    await delay(300)
    const { id } = params
    const data = await request.json()
  ...
  // 러닝 삭제
  http.delete('/api/running/:id', async ({ params }) => {
    await delay(300)
    const { id } = params
  ...
  http.get('/api/aggregate', async ({ request }) => {
    ...
    const totalRunningTime = aggregateData.reduce((sum, item) => sum + item.runningDuration, 0)

    const aggregate: Aggregate = {
      runningItemCount: aggregateData.length,
      totalLength: totalRunningLength,
      totalTime: totalRunningTime,
    }
